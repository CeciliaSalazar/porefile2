process ExtractSpeciesSeqs {
   ...
   *   later columns may contain "[S] Genus species"
   * Keep rows with rank 'S' OR "[S]" anywhere in the line.
   * Then subset the FASTA by normalized IDs.
   */
  ...
  # 2) Filter FASTA by those IDs (normalize headers: cut at space, |, /, drop trailing .<digits>)
  awk '
    function norm(s,t){
      t=s
      gsub(/^[ \t]+|[ \t]+$/,"",t)
