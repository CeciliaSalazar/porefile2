...
def parameters_expected = [
  ...
  'nanofilt_quality','nanofilt_length','nanofilt_maxlength',
  'nanofilt_headcrop','nanofilt_tailcrop',
  'yacrd_c','yacrd_n',
  ...
  'minimap2_k','minimap2_f','minimap2_x','minimap2_KM',
...
workflow {
  ...
  SetSilva()
  SetSilva.out.fasta    .set{ silva_fasta_ch }
  SetSilva.out.synonyms .set{ silva_synonyms_ch }
  ...
  MeganLca.out.collectFile(storeDir: "$params.outdir/Rma") { val, file ->
    ["${val}.rma", file]
  }

  GetReadInfo( MeganLca.out )
  GetReadInfo.out.set{ base_read_assingments_ch }

  // Publish read taxonomy assignments
  base_read_assingments_ch
    .collectFile(storeDir: "$params.outdir/Read_Assignments") { val, file ->
      [ "${val}.read_info", file ]
    }
 ...
 // === NUEVO BLOQUE: emparejar FASTA con read_info y extraer secuencias a nivel especie ===
  ...
  if ( !params.noSpeciesPolishing ){
    ...
    Polish.out.counts.collectFile(storeDir: "$params.outdir/"){ file -> [ "COUNTS_polished.tsv", file ] }
    Polish.out.taxcla.collectFile(storeDir: "$params.outdir/"){ file -> [ "TAXCLA_polished.tsv", file ] }
  }
}
...
def helpMessage() {
  ...
  Other:
    --silvaFasta                  Path to SILVA_*_SSURef_NR99_tax_silva.fasta.gz
    --silvaFastaURL               URL to SILVA fasta
    --silvaTaxNcbiSp              Path to tax_ncbi-species_ssu_ref_nr99_*.txt.gz
    --silvaTaxNcbiSpURL           URL to tax_ncbi-species
    --silvaTaxmap                 Path to taxmap_slv_ssu_ref_nr_*.txt.gz
    --silvaTaxmapURL              URL to taxmap
    --fullSilva                   Use full SILVA database
